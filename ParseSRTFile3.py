...
for item in items:
    ...
    if os.path.isdir(possibleDir):
        ...
        newFileName = item + ".txt"
        newFilePath = os.path.join(possibleDir, newFileName)
        print newFilePath
        ...
        for any_file in files:
            ...
            if os.path.isfile(filePath) and file_extension == ".srt":
                ...
                srtFile = open(filePath, "r")
                for line in srtFile:
                    if not exp.match(line) and not exp2.match(line) and not line.isspace():
                        newFile.write(line)
                # Close file
                srtFile.close()
